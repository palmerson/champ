    ...
    $.get( "api_product.php", { pid: pid } )
      .done(function( data ) {
        ...
        if($('#tg-0YX8L tr').length % 2 == 0){
          ...
          $html += '<td class=tg-vn4c><input class="inp qty" type=text name=qty[] ></td>';
          ...
          $html += '<td class=tg-031e><input class="inp qty" type=text name=qty[] ></td>';
...
function update_amounts()
{
    ...
    $('#tg-0YX8L > tbody  > tr').not(":first").each(function() {
      ...
      sum+=amount;
      $(this).find('.pd_amount').text(''+amount.toFixed(2));
      $(this).find('.pdm').val(amount.toFixed(2));
    });
